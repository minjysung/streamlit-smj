...
def  plotting_demo():
   ...
   # money = pd.read_csv("money_data7.csv")
    ...
    plt.subplot(222)
    plt.plot(  list( money['A_MONTH'] ), list( money['K_RATE'] ), color='salmon' , marker='o'     ) 
    plt.xticks(tuple(money['A_MONTH']) )
    plt.title('Korea rate')

    plt.subplot(223)
    plt.plot(  list( money['A_MONTH'] ), list( money['KOSPI'] ), color='lightskyblue' , marker='o'     ) 
    plt.xticks(tuple(money['A_MONTH']) )
